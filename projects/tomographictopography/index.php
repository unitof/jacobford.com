...
<?php htmlheader($pageClass, $middle, $bottom, true); ?>
	<div id="story">
		...
		<p>But humans are really bad at deriving meaning from grids of numbers, so the numbers are mapped to color values, using a black-to-white gradient. The lowest value is mapped to black, the highest to white, and all values in between are assigned corresponding shades of grey. The grid becomes an image; coordinates now pixels. It&rsquo;s our cultural instinct to represent data visually, but the habit has consequences.</p>
		...
		<div class="overhang">
			<img src="gradientcompare@2x.png" alt="Values from 0 to 1 are distributed along two gradients: one goes from black (0) to white (1). The other begins black (0), then cycles through violet, blue, green, yellow, orange, and then red (1). Samples are taken at the values 0.4 and 0.5 on each e">
		</div>
		<p>So we began to use color. When we map values along a gradient with multiple color stops, close values become much easier to distinguish. If we switch from a black-to-white gradient to one which begins at black then cycles through the rainbow of the visible spectrum, those same values of 0.4 and 0.5 are the now the difference between green and blue, instead of medium grey and ligther grey. Alternatives to the rainbow gradient were developed which used different colors in different orders, or interspersed bands within the gradient to aid in differentiation, and eventually it gets completely out of hand.</p>
		<div class="overhang">
			<div class="gallery">
		...
		<p>So I ran the result of a PET scan through software designed for converting DEM images into 3D elevation models, and got the terrain of a working brain. The cool thing about 3D models is that they can be 3D-printed, and brain activity can be made tactile.</p>
		<div class="overhang">
		...
		<p>This project does not exist to say everything we&rsquo;ve been doing to image the brain is wrong, but to ask why we&rsquo;ve chosen our sense of vision to be the authoritative experience of the world and its data. Why not also trust in our sense of touch?</p>
		<div class="overhang">
